...
SPORT_WORDS = (
    ...
    "|밸리댄스|라인댄스|댄스|배드민턴|탁구|테니스|골프|파크골프|볼링|당구|족구"
...
#   "도자기공예"  -> '기공'   (도자기·공예)
#   "금정구평생학습관" -> '정구'  (금정구)
#   "자전거 탄 소년(12세 관람가)" -> '자전거'  (영화 상영)
# 그래서 종목 단어를 줄이는 대신, 운동이 아닌 주제를 명시해서 먼저 떨어뜨린다.
EXCLUDE_RE = re.compile(
    r"운동회|자원봉사|운동장 대관|스포츠마케팅|스포츠경영"
    # 자격증·지도자 양성 과정은 수강이 아니라 직업 훈련이다
    r"|자격증|지도사|지도자|양성과정|양성 과정"
    # 공예·예술·교양
    r"|공예|도자기|서예|미술|그림|사진|합창|악기|우쿨렐레|풍물|난타"
    # 영화·독서·인문
    r"|영화|상영|시네마|관람가|독서|글쓰기|인문학|한국사|강연|특강"
    # 생활·직무 교육
    r"|요리|제과|제빵|바리스타|코딩|컴퓨터|스마트폰|외국어|재테크|투자"
)

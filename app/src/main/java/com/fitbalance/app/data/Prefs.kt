...
/** 설정 화면에서 고를 수 있는 위치 프리셋 (지도 화면은 이번 범위 밖이라 좌표를 직접 고른다). */
data class PlacePreset(val name: String, val lat: Double, val lng: Double)

val PLACE_PRESETS = listOf(
    PlacePreset("시청·광화문", 37.5665, 126.9780),
    PlacePreset("강남역", 37.4979, 127.0276),
    PlacePreset("여의도", 37.5219, 126.9245),
    PlacePreset("판교", 37.3947, 127.1112),
    PlacePreset("홍대·마포", 37.5563, 126.9236),
    PlacePreset("잠실", 37.5133, 127.1000),
    PlacePreset("성수·왕십리", 37.5445, 127.0557),
)

...
def recommend(req: RecommendRequest):
    ...
    try:
        ...
        # 이 데이터가 무엇인지 밝힌다.
        # 전국평생학습강좌표준데이터는 분기 갱신이라 오늘 기준으로 아직 안 끝난
        # 강좌는 7%뿐이다. 그것만 쓰면 전국에 305줄만 남아 대부분의 도시가 0건이
        # 된다. 그래서 최근 1년 안에 운영된 정기 프로그램까지 포함한다.
        # 사용자가 헛걸음하지 않도록 그 사실을 숨기지 않는다.
        if top:
            response["data_notice"] = (
                "공공데이터에 등록된 최근 1년 이내 개설 강좌입니다. "
                "학기마다 다시 열리는 강좌가 많지만, 방문 전 기관에 개설 여부를 확인하세요."
            )
        if not top:
